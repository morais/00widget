...
`WIDGET_PUSH_APNS_DIAGNOSTICS` defaults to `"false"`. Setting it to `"true"`
persists the latest final APNs status, reason, id, attempt count, and timestamp
...
| `widget_push_delivery_diagnostics` | `token` | Optional latest final WidgetKit APNs result per token; written only while `WIDGET_PUSH_APNS_DIAGNOSTICS` is `"true"`. |
...
- **WidgetKit push** — `apns-push-type: widgets`, `apns-topic: <bundleId>.push-type.widgets`, `content-changed: true`, a short expiration, and a stable collapse id because every reload fetches the latest card state. Batch upserts make one reload decision for all cards, and reloads are bounded per widget by a token bucket — at least 5 minutes apart, six available back-to-back, one refilling every 30 minutes — because Apple budgets reloads per widget instance and counts pushes against the same allowance as the widget's own refreshes. A bucket rather than a daily quota so a busy morning cannot leave the widget dark all afternoon. Changes arriving inside that window update one durable `widget_push_pending` row; one delayed Cloudflare Queue message delivers the latest coalesced state when the window opens. Permanent token failures are pruned; transient APNs failures use bounded in-request retries and retain and retry the coalesced row. An empty queue does not invoke the Worker. Failures and sampled successes still go to logs; the opt-in `WIDGET_PUSH_APNS_DIAGNOSTICS` flag additionally keeps the latest final result per token in D1 and exposes it through `/v1/status` and the admin token table.
