...
export async function handleIntegrationMd(req: Request): Promise<Response> {
  const baseURL = new URL(req.url).origin;
  return new Response(renderHostedIntegrationMarkdown(baseURL), {
    status: 200,
    headers: {
...
function renderHostedIntegrationMarkdown(baseURL: string): string {
  return integrationMarkdown
    .replace(
    ...
    .replace(
      ...
      `## Configure access\n\nThis hosted integration document is being served by the 00Widget Worker you should call, so do not ask the operator for the base URL. Use:\n\n\`\`\`sh\n00WIDGET_BASE_URL=${baseURL}\n\`\`\`\n\nAsk the operator only for the API key:\n\n| Env var              | Example              | Where it comes from                         |\n| -------------------- | -------------------- | ------------------------------------------- |\n| \`00WIDGET_API_KEY\`   | a long random string | A tenant API token generated by the operator |`,
    )
    .replace(
