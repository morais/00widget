...
2. For each, pick a template (`summary`, `progress`, `list`, `action`, `chart`, `history`, `breakdown`, `briefing`, or `timeline`) per llms.md's decision matrix.
...
- **DashboardCard** — a single widget tile. Templates: `summary`, `progress`, `list`, `action`, `chart`, `history`, `breakdown`, `briefing`, `timeline`.
