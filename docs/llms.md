...
| `00WIDGET_WEBHOOK_MANAGER_KEY` | a separate random string | Optional token with `webhook:manage`, needed only to administer actions |
...
Tokens are capability-scoped. The normal agent publisher token has `tenant:read`
and `publish`; it can read and publish cards and Live Activities, but cannot
register device push tokens, run actions, manage shares, or administer the
action webhook. Those operations use separate device, app-only, or webhook
manager credentials. A `403` naming a required scope means the operator must
issue the right credential; do not try to work around it with another endpoint.
...
Before buttons can call your system, ask the operator for a credential with the
`webhook:manage` scope and register the webhook with that dedicated credential:

```sh
curl -X PUT "$00WIDGET_BASE_URL/v1/integrations/webhook" \
  -H "Authorization: Bearer $00WIDGET_WEBHOOK_MANAGER_KEY" \
  -H "Content-Type: application/json" \
...
To disable actions, call `DELETE /v1/integrations/webhook` with the same
webhook-manager credential.
