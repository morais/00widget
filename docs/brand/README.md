...
`app-icon-master.png` is the exact approved playful artwork and the single
visual source for platform exports. `mark-transparent-master.png` is *generated*
from it — the same artwork with the navy backdrop cut away — and is what every
transparent export is fitted from. Do not replace it with a flattened copy of
the opaque master: each consumer of it composites the mark over its own
background, so a baked-in backdrop shows up as a box around the mark.
Platform exports are generated by:
