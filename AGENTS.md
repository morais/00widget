...
- **The scrolling column's top edge is a fade, not a cut, and the inset exists to keep it invisible at rest.** A television that slices a row of cards in half against an invisible line reads as broken; Apple TV apps fade content out as it passes under the title instead. `TVCardMetrics.topFade` is both the height of the gradient mask on the scroll view and the column's own top inset, and the two being the same number is the point: at rest the first thing in the column — the section label — begins where the fade has finished, so nothing on a still screen is dimmed and the gradient is only ever seen by content moving under it. Change one and change the other. The mask is render-only, so it costs the focus engine nothing: a masked view is still focusable, which matters on a screen navigated by moving focus rather than by pointing. Neither capture shows it, because at rest there is nothing under the fade — check it by scrolling on a device, or render a card offset under the mask the way `TVRenderProbe` can.
