...
- **`FakeD1` proves logic, not bindings.** It reads the bound array positionally, so a statement whose placeholders and values are misaligned passes the suite and fails against D1 — and a conditional upsert that wrongly returns false is silent, since "no rows changed" is a legitimate outcome rather than an error. The widget reload claim is the live example: it binds five values twelve times. Prefer plain `?` there over numbered `?NNN`, keep the bind order commented, and check a new statement of that shape against the real database rather than trusting a green suite.
