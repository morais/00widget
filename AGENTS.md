...
- **The MCP endpoint's discovery methods are deliberately anonymous.** ChatGPT fetches `tools/list` with no credential; gating it leaves the connector with no tools to offer. The list is generated from static schemas, is identical for every tenant, and describes exactly what `/llms.md` already publishes. Only `tools/call` requires a credential — that is where the 401 challenge belongs, because it is the point where a client can act on it. A credential that is *present but invalid* is always an error, on every method: silently degrading a bad token to anonymous access would make a revoked connector look like a working one.
